...
class BaseAbility(BaseAction, ABC):
    ...
    Example:
        ```python
        # This is abstract - use concrete subclasses
        from abilities.ability_offensive import OffensiveAbility
    ...
    def get_damage_expr(self, actor: Any, damage_components: list[DamageComponent]) -> str:
        """
        ...
        Example:
            ...
            damage_expr = ability.get_damage_expr(dragon, ability.damage)
        ...
        return " + ".join(
            substitute_variables(component.damage_roll, actor.get_expression_variables())
            for component in damage_components
        )

    def get_min_damage(self, actor: Any, damage_components: list[DamageComponent]) -> int:
        """
        ...
        Example:
            ```python
            # For "3d8 + 4" damage
            min_dmg = ability.get_min_damage(character, ability.damage)
            # Returns: 7 (3*1 + 4)
            ```
        """
        if not damage_components:
    ...
    def get_max_damage(self, actor: Any, damage_components: list[DamageComponent]) -> int:
        """
        ...
        Example:
            ```python
            # For "3d8 + 4" damage
            max_dmg = ability.get_max_damage(character, ability.damage)
            # Returns: 28 (3*8 + 4)
            ```
        """
        if not damage_components:
