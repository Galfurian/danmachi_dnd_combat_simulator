...
class DataValidator:
    ...
    def _validate_field(
        self, field_name: str, value: Any, validator: FieldValidator
    ) -> ValidationResult:
        ...
        if validator.allowed_values and value not in validator.allowed_values:
            result.add_error(
                f"Field '{field_name}' must be one of: {validator.allowed_values}"
            )
...
CHARACTER_SCHEMA = DataValidator(
    {
        "name": FieldValidator(
            required=True, field_type=str, min_length=1, max_length=50
        ),
        "type": FieldValidator(
            required=True, field_type=str, allowed_values=["PLAYER", "ENEMY", "ALLY"]
        ),
        ...
        "total_hands": FieldValidator(
            required=False, field_type=int, min_value=0, max_value=10
        ),
        "number_of_attacks": FieldValidator(
            required=False, field_type=int, min_value=1, max_value=10
        ),
    }
)
...
STATS_SCHEMA = DataValidator(
    {
        "strength": FieldValidator(
            required=True, field_type=int, min_value=1, max_value=30
        ),
        "dexterity": FieldValidator(
            required=True, field_type=int, min_value=1, max_value=30
        ),
        "constitution": FieldValidator(
            required=True, field_type=int, min_value=1, max_value=30
        ),
        "intelligence": FieldValidator(
            required=True, field_type=int, min_value=1, max_value=30
        ),
        "wisdom": FieldValidator(
            required=True, field_type=int, min_value=1, max_value=30
        ),
        "charisma": FieldValidator(
            required=True, field_type=int, min_value=1, max_value=30
        ),
    }
)
...
WEAPON_SCHEMA = DataValidator(
    {
        ...
        "hands_required": FieldValidator(
            required=False, field_type=int, min_value=0, max_value=5
        ),
