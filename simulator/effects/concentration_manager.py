...
class ConcentrationManager:
    ...
    def __init__(self, owner: Any):
        ...
        self.concentration_spells: Dict[str, ConcentrationSpell] = {}  # spell_name -> ConcentrationSpell
    ...
    def add_concentration_effect(self, spell: Spell, target: Any, active_effect: Any, mind_level: int) -> bool:
        """
    ...
    def drop_oldest_concentration_spell(self) -> None:
        ...
        if len(target_names) == 1:
            cprint(f"    :no_entry: [bold yellow]{oldest_conc_spell.spell.name}[/] concentration broken on [bold]{target_names[0]}[/] (concentration limit reached).")
        else:
            targets_str = ', '.join(target_names)
            cprint(f"    :no_entry: [bold yellow]{oldest_conc_spell.spell.name}[/] concentration broken on [bold]{targets_str}[/] (concentration limit reached).")
    ...
    def break_concentration(self, spell: Optional[Spell] = None) -> bool:
        ...
        if spell:
            ...
            if spell_key in self.concentration_spells:
                ...
                if len(target_names) == 1:
                    cprint(f"    :no_entry: [bold yellow]{spell.name}[/] concentration broken on [bold]{target_names[0]}[/].")
                else:
                    targets_str = ', '.join(target_names)
                    cprint(f"    :no_entry: [bold yellow]{spell.name}[/] concentration broken on [bold]{targets_str}[/].")
            ...
            if self.concentration_spells:
                ...
                if len(spell_info) == 1:
                    ...
                    targets_str = ', '.join(target_names)
                    cprint(f"    :no_entry: [bold yellow]{spell_name}[/] concentration broken on [bold]{targets_str}[/].")
                else:
                    spell_descriptions = [f"{name} on {', '.join(targets)}" for name, targets in spell_info]
                    cprint(f"    :no_entry: All concentration broken: [bold yellow]{'; '.join(spell_descriptions)}[/].")
